...
class trees(object):
    ...
    def calcShannonEnt(self,Dataset):
        #计算香农熵H(p)=-∑pi*log(pi) ,
        #the enpropy is the measure of uncertain of random variables
    ...
    def chooseBestFeature2Split(self,dataSet):
              ''' choose the best Feature from all column of dataSet,which 
                  can split dataset with minmal infomation gain: g(D,A)=H(D)-H(D|A)    
              '''
              numEntires = len(dataSet)
              numFeat = len(dataSet[0]) - 1  # the last column is label
              bestGain = 0.0;bestFeature=-1
              baseEnt = self.calcShannonEnt(dataSet)  # get H(D)—denoting the uncertain of classifing of dataSet
              for i in range(numFeat):
                     ...
                     for ul in uniqueLabel:  #为每一个取值都计算信息增益 
                           ...
                           newEntropy+=ratio * self.calcShannonEnt(subSet)   #H(D|A) = ∑Di/D H(Di) 
    ...
    # 
    def createTree(self,dataSet,labels):   #ID3   
      ...
      classList = [example[-1] for example in dataSet] #get last column
      ...
      myTree = {bestFeatLab:{}}
      del(labels[bestFeature]) # 将已经使用的特征删除,labels存放特征的名字
